...
class ArangoRequestError(Exception):
    ...
    :param response: the ArangoResponse object
...
class VersionGetError(ArangoRequestError):
    """Failed to get the ArangoDB version."""
...
class SimpleQueryRemoveByKeysError(ArangoRequestError):
    """Failed to execute the ``remove-by-keys`` simple query."""
...
class TransactionError(ArangoRequestError):
...
class VertexCollectionAllError(ArangoRequestError):
