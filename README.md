...
a.name          # equivalent to a.database("_system").name
a.collections   # equivalent to a.database("_system").collections
a.id            # equivalent to a.database("_system").id
a.path          # equivalent to a.database("_system").path
a.is_system     # equivalent to a.database("_system").is_system

# Get information on a specific database
a.database("db01").collections
a.database("db02").id
a.database("db03").path
a.database("dbXX").is_system
...
a.collection("my_col").create_document({"value" : 1})
a.{whatever}

# Working with database "my_db"
a.database("my_db").create_collection("my_col")
a.database("my_db").collection("my_col").create_document({"value": 1})
a.database("my_db").{whatever}
```
...
my_db.create_collection("new_ecol", is_edge=True)
...
my_col = a.database("my_db").collection("my_col")
...
```python
my_col = a.collection("my_col")  # or a.collection("mycol")
...
```python
my_graph = a.database("my_db").graph("my_graph")
...
action = """
  function () {
      var database = require('internal').database;
      database.col01.save({ _key: 'doc01'});
      database.col02.save({ _key: 'doc02'});
      return 'success!';
  }
"""
